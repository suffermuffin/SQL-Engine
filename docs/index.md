# Documentation Index

Welcome to the documentation of `sqlengine`, a Python library designed to simplify database operations with SQLite.

## Table of Contents

1. **API**
   - [Overview](api.md)

2. **Table Declaration**
   - [Overview](table_declaration.md)
   - [Class Declaration](table_declaration.md#class-declaration)
   - [Class Declaration with Annotation](table_declaration.md#class-declaration-with-annotation)
   - [Schema Declaration](table_declaration.md#schema-declaration)
   - [Table Instantiation](table_declaration.md#table-instantiation)

3. **Statements**
   - [Overview](statements.md)
   - [Statements](statements.md#1-statements)
   - [Where](statements.md#11-where)
   - [Select](statements.md#12-select)
   - [Update](statements.md#13-update)
   - [Delete](statements.md#14-delete)

4. **Transactions**
   - [Overview](transactions.md)
   - [Transaction per Table](transactions.md#transaction-per-table)
   - [Shared Connection](transactions.md#shared-connection)
   - [Manually Managed Connections](transactions.md#manually-managed-connections)

5. **Syntax Sugar**
   - [Overview](syntax_sugar.md)
   - [Single Index](syntax_sugar.md#single-index)
   - [Multi Index](syntax_sugar.md#multi-index)
   - [Length and Shape](syntax_sugar.md#length-and-shape)

6. **Custom Types**
   - [Overview](custom_types.md)

7. **Examples**
   - [Overview](examples.md)


Each section provides information and examples on how to use specific features of `sqlengine`. For more advanced usage, refer to the source code or additional resources provided in the repository.
